...
namespace TMDT_BanHang.Areas.Admin.Controllers
{
    public class OrdersController : BaseController
    {
            ...
            if (User.Identity.IsAuthenticated)
            {
                if (User.Identity.Permiss_Update() == true || User.Identity.Permiss_Modify() == true)
                {
                    ...
                    if (order != null)
                    {
                        ...
                        db.Entry(order).State = EntityState.Modified;
                        db.SaveChanges();
                        Notification.set_flash("Đã chuyển trạng thái đơn hàng: " + "H&H" + id + " sang chờ xử lý!", "success");
                    }
                    else
                    {
        ...
        string OrderPhone,string OrderName,string OrderAddress,string OrderDelivery,string DeliveryId, string emailFor)
        {
            ...
            else if (emailFor == "ChangeComplete")
            {
                ...
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được giao thành công. Cảm ơn bạn đã tin tưởng Sữa bột tốt H&H, hi vọng trong tương lai bạn sẽ tiếp tục sử dụng sản phẩm  của Chúng tôi.");
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink); 
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{UserEmail}}", emailID);
            ...
            else if (emailFor == "CancleOrders")
            {
                ...
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được hủy theo yêu cầu của bạn. Sữa bột tốt H&H bắt đầu tiến hành thủ tục hoàn tiền và sẽ thông tin đến bạn khi đã hoàn tất.");
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink);
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{UserEmail}}", emailID);
