...
using TMDT_BanHang.PaymentLibrary;
using TMDT_BanHang.ZaloPay;
using TMDT_BanHang.ZaloPay.Models;
using log4net;
using System;
    ...
    {        
        ...
        public async Task<ActionResult> SaveOrder(ChekcoutOrderDTO chekcout, OrderAddress orderAddress, string check_discount, string SubjectMail, string ButtonConfirmlink,string ButtonConfirm,string OrderPayment, string OrderDelivery,string DeliveryId, string SubOrderTotal,string Discount_Price,string ProductOrder, string OrderId,string OrderStatus, string OrderTotal, string UserEmail, string UserName, string UserPhoneNumber, string UserAddress)
        {
            ...
            try
            {
                ...
                //DateTime create_at = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, vnTimeZone);
                var listProduct = new List<Product>();
                var order = new Order()
                {
                    account_id = User.Identity.GetUserId(),
                    create_at = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, vnTimeZone),
                    create_by = User.Identity.GetUserId().ToString(),
                    status = "1",
                    order_note = Request.Form["OrderNote"].ToString(),
                    delivery_id = 1,
                    oder_date = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, vnTimeZone),
                    update_at = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, vnTimeZone),
                ...
                for (int i = 0; i < cart.Item1.Count; i++)
                {
                    ...
                    if (item.Discount != null)
                    {
                        ...
                        if (item.Discount.discount_start < TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, vnTimeZone) && item.Discount.discount_end > TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, vnTimeZone))
                        {
