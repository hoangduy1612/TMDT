...
namespace TMDT_BanHang.Controllers
{
    public class PaymentMethodsController : Controller
    {
        ...
        public ActionResult vnpay(object sender, EventArgs e)
        {
            ...
            //Get payment input
            OrderInfo order = new OrderInfo();
            var orderId = Convert.ToInt64(TempData["OrderId"]);
            var re_order = db.Order_Detail.Where(m => m.order_id == orderId);
            var sum  = 0.0;
            foreach(var i in re_order)
            {
            ...
            order.Amount = sum;  // Giả lập số tiền thanh toán hệ thống merchant gửi sang VNPAY 100,000 VND
            order.Status = "0"; //0: Trạng thái thanh toán "chờ thanh toán" hoặc "Pending"
            order.CreatedDate = DateTime.Now;
            ...
            // Định dạng đúng yêu cầu VNPAY: yyyyMMddHHmmss
