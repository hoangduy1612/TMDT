...
namespace TMDT_BanHang.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> SignIn(Sigin model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                ...
                // ❗ Always clear the token as it's single-use
            ...
            // ✅ Truy vấn tài khoản
            var DataItem = db.Accounts.SingleOrDefault(m => m.status == "1" && m.Email.ToLower() == model.Email && m.password == model.password);
            var checkdisable = db.Accounts.SingleOrDefault(m => m.status == "2" && m.Email.ToLower() == model.Email && m.password == model.password);
            var checkactivate = db.Accounts.SingleOrDefault(m => m.status == "0" && m.Email.ToLower() == model.Email && m.password == model.password);

            bool create = false, read = false, update = false, delete = false, access = false, modify = false;

            if (checkdisable != null)
            {
                Notification.set_flash1s("Tài khoản đã bị vô hiệu hoá", "danger");
            }
            else if (checkactivate != null)
            {
            ...
            else if (DataItem != null)
            {
                ...
                {
                    if (permiss.permission_id == 1) create = true;
                    if (permiss.permission_id == 2) read = true;
                    if (permiss.permission_id == 3) modify = true;
                    if (permiss.permission_id == 7) update = true;
                    if (permiss.permission_id == 4) delete = true;
                    if (permiss.permission_id == 5) access = true;
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register([Bind(Exclude = "status,Requestcode")] Account accounts,Register model)
        {
            if (ModelState.IsValid)
            {
                // ✅ Validate reCAPTCHA
